...
{if $numItems}
{capture assign='uniqueID'}tag:{$baseurl|replace:'http://':''|replace:'/':''},{$items[0].createdDate|dateformat|default:$smarty.now|dateformat:'%Y-%m-%d'}:{modurl modname='MUVideo' type=$lct func='display' id=$items[0].id ot='collection'}{/capture}
    <id>{$uniqueID}</id>
    <updated>{$items[0].updatedDate|default:$smarty.now|dateformat:'%Y-%m-%dT%H:%M:%SZ'}</updated>
{/if}
    <link rel="alternate" type="text/html" hreflang="{lang}" href="{modurl modname='MUVideo' type=$lct func='main' fqurl=1}" />
...
{foreach item='collection' from=$items}
    <entry>
        ...
        <link rel="alternate" type="text/html" href="{modurl modname='MUVideo' type=$lct func='display' id=$collection.id ot='collection' fqurl='1'}" />
        {capture assign='uniqueID'}tag:{$baseurl|replace:'http://':''|replace:'/':''},{$collection.createdDate|dateformat|default:$smarty.now|dateformat:'%Y-%m-%d'}:{modurl modname='MUVideo' type=$lct func='display' id=$collection.id ot='collection'}{/capture}
        <id>{$uniqueID}</id>
