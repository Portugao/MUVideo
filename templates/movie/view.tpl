...
<div class="muvideo-movie muvideo-view">
    ...
    {/if}
        <table class="z-datatable">
            ...
            <tr>
                ...
                <th id="hTitle" scope="col" class="z-left">
                    {sortlink __linktext='Title' currentsort=$sort modname='MUVideo' type=$lct func='view' sort='title' sortdir=$sdir all=$all own=$own collection=$collection workflowState=$workflowState searchterm=$searchterm pageSize=$pageSize ot='movie'}
                </th>
                ...
                <th id="hCollection" scope="col" class="z-left">
                    {sortlink __linktext='Collection' currentsort=$sort modname='MUVideo' type=$lct func='view' sort='collection' sortdir=$sdir all=$all own=$own collection=$collection workflowState=$workflowState searchterm=$searchterm pageSize=$pageSize ot='movie'}
                </th>
            ...
            <tr class="{cycle values='z-odd, z-even'}">
                ...
                <td headers="hCollection" class="z-left">
                    {if isset($movie.Collection) && $movie.Collection ne null}
                        ...
                        {/strip}</a>
                        <a id="collectionItem{$movie.id}_rel_{$movie.Collection.id}Display" href="{modurl modname='MUVideo' type=$lct func='display' id=$movie.Collection.id ot='collection' theme='Printer' forcelongurl=true}" title="{gt text='Open quick view window'}" class="z-hide">{icon type='view' size='extrasmall' __alt='Quick view'}</a>
        ...
        {if !isset($showAllEntries) || $showAllEntries ne 1}
            {pager rowcount=$pager.numitems limit=$pager.itemsperpage modname='MUVideo' type=$lct func='view' ot='movie' lct=$lct}
        {/if}
    {if $lct eq 'admin'}
            <fieldset>
