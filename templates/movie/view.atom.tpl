...
{if $numItems}
{capture assign='uniqueID'}tag:{$baseurl|replace:'http://':''|replace:'/':''},{$items[0].createdDate|dateformat|default:$smarty.now|dateformat:'%Y-%m-%d'}:{modurl modname='MUVideo' type=$lct func='display' id=$items[0].id ot='movie'}{/capture}
    <id>{$uniqueID}</id>
...
{foreach item='movie' from=$items}
    <entry>
        ...
        <link rel="alternate" type="text/html" href="{modurl modname='MUVideo' type=$lct func='display' id=$movie.id ot='movie' fqurl='1'}" />
        {capture assign='uniqueID'}tag:{$baseurl|replace:'http://':''|replace:'/':''},{$movie.createdDate|dateformat|default:$smarty.now|dateformat:'%Y-%m-%d'}:{modurl modname='MUVideo' type=$lct func='display' id=$movie.id ot='movie'}{/capture}
        <id>{$uniqueID}</id>
