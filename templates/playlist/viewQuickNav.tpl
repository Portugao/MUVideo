...
{assign var='objectType' value='playlist'}
<form action="{$modvars.ZConfig.entrypoint|default:'index.php'}" method="get" id="mUVideoPlaylistQuickNavForm" class="muvideo-quicknav">
    <fieldset>
        ...
        <input type="hidden" name="ot" value="playlist" />
        ...
        {/if}
        <input type="submit" name="updateview" id="muvideo_playlistquicknav_updateview" value="{gt text='OK'}" />
    </fieldset>
</form>
<script type="text/javascript">
/* <![CDATA[ */
    document.observe('dom:loaded', function() {
        mUMUVideoInitQuickNavigation('playlist');
        {{if isset($searchFilter) && $searchFilter eq false}}
            {{* we can hide the submit button if we have no quick search field *}}
            $('muvideo_playlistquicknav_updateview').addClassName('z-hide');
        {{/if}}
    });
/* ]]> */
</script>
