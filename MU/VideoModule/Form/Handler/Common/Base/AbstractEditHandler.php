...
namespace MU\VideoModule\Form\Handler\Common\Base;
...
use MU\VideoModule\Helper\HookHelper;
...
abstract class AbstractEditHandler
{
    ...
    public function processForm(array $templateParameters)
    {
        ...
        if (null === $this->returnTo) {
            ...
            if ($this->request->getSession()->has('muvideomoduleReferer')) {
                $this->returnTo = $this->request->getSession()->get('muvideomoduleReferer');
            } elseif ($this->request->headers->has('muvideomoduleReferer')) {
                $this->returnTo = $this->request->headers->get('muvideomoduleReferer');
                $this->request->getSession()->set('muvideomoduleReferer', $this->returnTo);
        ...
        $this->permissionComponent = 'MUVideoModule:' . $this->objectTypeCapital . ':';
        ...
        if ($this->templateParameters['mode'] == 'edit') {
            ...
            if (null !== $entity) {
                if (true === $this->hasPageLockSupport && $this->kernel->isBundle('ZikulaPageLockModule') && null !== $this->lockingApi) {
                    ...
                    $lockName = 'MUVideoModule' . $this->objectTypeCapital . $this->createCompositeIdentifier();
        ...
        if (false === $actions || !is_array($actions)) {
            ...
            $logArgs = ['app' => 'MUVideoModule', 'user' => $this->currentUserApi->get('uname'), 'entity' => $this->objectType, 'id' => $entity->createCompositeIdentifier()];
        ...
        if ($isRegularAction || $action == 'delete') {
            ...
            if ($entity->supportsHookSubscribers()) {
                ...
                if ($action != 'delete') {
                    ...
                    $url = new RouteUrl('muvideomodule_' . $this->objectType . '_display', $urlArgs);
                }
    ...
    protected function addDefaultMessage($args, $success = false)
    {
        ...
        $logArgs = ['app' => 'MUVideoModule', 'user' => $this->currentUserApi->get('uname'), 'entity' => $this->objectType, 'id' => $this->entityRef->createCompositeIdentifier()];
        if (true === $success) {
