...
abstract class AbstractEditHandler extends EditHandler
{
    ...
    public function processForm(array $templateParameters)
    {
        ...
        if ($this->templateParameters['mode'] == 'create') {
            if (!$this->modelHelper->canBeCreated($this->objectType)) {
                $this->request->getSession()->getFlashBag()->add('error', $this->__('Sorry, but you can not create the playlist yet as other items are required which must be created before!'));
    ...
    protected function getRedirectCodes()
    {
        ...
        // user index page of playlist area
        $codes[] = 'userIndex';
        // admin index page of playlist area
        $codes[] = 'adminIndex';
        // user list of playlists
        $codes[] = 'userView';
        // admin list of playlists
        $codes[] = 'adminView';
        // user list of own playlists
        $codes[] = 'userOwnView';
        // admin list of own playlists
        $codes[] = 'adminOwnView';
        // user detail page of treated playlist
        $codes[] = 'userDisplay';
    ...
    protected function getDefaultMessage(array $args = [], $success = false)
    {
        ...
        switch ($args['commandName']) {
            case 'submit':
                if ($this->templateParameters['mode'] == 'create') {
                    $message = $this->__('Done! Playlist created.');
                } else {
                    $message = $this->__('Done! Playlist updated.');
                }
                break;
            case 'delete':
                $message = $this->__('Done! Playlist deleted.');
                break;
            default:
                $message = $this->__('Done! Playlist updated.');
                break;
