...
abstract class AbstractUserListener implements EventSubscriberInterface
{
    ...
    public function delete(GenericEvent $event)
    {
        ...
        $logArgs = ['app' => 'MUVideoModule', 'user' => $this->currentUserApi->get('uname'), 'entities' => 'collections'];
        $this->logger->notice('{app}: User {user} has been deleted, so we deleted/updated corresponding {entities}, too.', $logArgs);
        
        $repo = $this->entityFactory->getRepository('movie');
        // set creator to admin (UsersConstant::USER_ID_ADMIN) for all movies created by this user
        $repo->updateCreator($userId, UsersConstant::USER_ID_ADMIN, $this->translator, $this->logger, $this->currentUserApi);
        
        // set last editor to admin (UsersConstant::USER_ID_ADMIN) for all movies updated by this user
        $repo->updateLastEditor($userId, UsersConstant::USER_ID_ADMIN, $this->translator, $this->logger, $this->currentUserApi);
        
        $logArgs = ['app' => 'MUVideoModule', 'user' => $this->currentUserApi->get('uname'), 'entities' => 'movies'];
        $this->logger->notice('{app}: User {user} has been deleted, so we deleted/updated corresponding {entities}, too.', $logArgs);
        
        $repo = $this->entityFactory->getRepository('playlist');
        // set creator to admin (UsersConstant::USER_ID_ADMIN) for all playlists created by this user
        $repo->updateCreator($userId, UsersConstant::USER_ID_ADMIN, $this->translator, $this->logger, $this->currentUserApi);
        
        // set last editor to admin (UsersConstant::USER_ID_ADMIN) for all playlists updated by this user
        $repo->updateLastEditor($userId, UsersConstant::USER_ID_ADMIN, $this->translator, $this->logger, $this->currentUserApi);
        
        $logArgs = ['app' => 'MUVideoModule', 'user' => $this->currentUserApi->get('uname'), 'entities' => 'playlists'];
        $this->logger->notice('{app}: User {user} has been deleted, so we deleted/updated corresponding {entities}, too.', $logArgs);
    }
}
