...
use MU\VideoModule\VideoEvents;
use MU\VideoModule\Event\FilterCollectionEvent;
use MU\VideoModule\Event\FilterMovieEvent;
use MU\VideoModule\Event\FilterPlaylistEvent;
...
abstract class AbstractEntityLifecycleListener implements EventSubscriber, ContainerAwareInterface
{
    ...
    public function preRemove(LifecycleEventArgs $args)
    {
        ...
        $filterEventClass = '\\MU\\VideoModule\\Event\\Filter' . ucfirst($entity->get_objectType()) . 'Event';
        $event = new $filterEventClass($entity);
        $this->container->get('event_dispatcher')->dispatch(constant('\\MU\\VideoModule\\VideoEvents::' . strtoupper($entity->get_objectType()) . '_PRE_REMOVE'), $event);
        if ($event->isPropagationStopped()) {
            return false;
        }
        
        // delete workflow for this entity
        $workflowHelper = $this->container->get('mu_video_module.workflow_helper');
    ...
    public function postRemove(LifecycleEventArgs $args)
    {
        ...
        $uploadHelper = $this->container->get('mu_video_module.upload_helper');
        ...
        $logArgs = ['app' => 'MUVideoModule', 'user' => $this->container->get('zikula_users_module.current_user')->get('uname'), 'entity' => $objectType, 'id' => $objectId];
        $logger->debug('{app}: User {user} removed the {entity} with id {id}.', $logArgs);
        
        // create the filter event and dispatch it
        $filterEventClass = '\\MU\\VideoModule\\Event\\Filter' . ucfirst($objectType) . 'Event';
        $event = new $filterEventClass($entity);
        $this->container->get('event_dispatcher')->dispatch(constant('\\MU\\VideoModule\\VideoEvents::' . strtoupper($objectType) . '_POST_REMOVE'), $event);
    }
    ...
    public function prePersist(LifecycleEventArgs $args)
    {
        ...
        $filterEventClass = '\\MU\\VideoModule\\Event\\Filter' . ucfirst($entity->get_objectType()) . 'Event';
        $event = new $filterEventClass($entity);
        $this->container->get('event_dispatcher')->dispatch(constant('\\MU\\VideoModule\\VideoEvents::' . strtoupper($entity->get_objectType()) . '_PRE_PERSIST'), $event);
        if ($event->isPropagationStopped()) {
            return false;
        }
    }
    ...
    public function postPersist(LifecycleEventArgs $args)
    {
        ...
        $objectId = $entity->createCompositeIdentifier();
        $logger = $this->container->get('logger');
        $logArgs = ['app' => 'MUVideoModule', 'user' => $this->container->get('zikula_users_module.current_user')->get('uname'), 'entity' => $entity->get_objectType(), 'id' => $objectId];
        $logger->debug('{app}: User {user} created the {entity} with id {id}.', $logArgs);
        
        // create the filter event and dispatch it
        $filterEventClass = '\\MU\\VideoModule\\Event\\Filter' . ucfirst($entity->get_objectType()) . 'Event';
        $event = new $filterEventClass($entity);
        $this->container->get('event_dispatcher')->dispatch(constant('\\MU\\VideoModule\\VideoEvents::' . strtoupper($entity->get_objectType()) . '_POST_PERSIST'), $event);
    }
    ...
    public function preUpdate(PreUpdateEventArgs $args)
    {
        ...
        $this->container->get('event_dispatcher')->dispatch(constant('\\MU\\VideoModule\\VideoEvents::' . strtoupper($entity->get_objectType()) . '_PRE_UPDATE'), $event);
        if ($event->isPropagationStopped()) {
            return false;
        }
    }
    ...
    public function postUpdate(LifecycleEventArgs $args)
    {
        ...
        $filterEventClass = '\\MU\\VideoModule\\Event\\Filter' . ucfirst($entity->get_objectType()) . 'Event';
        $event = new $filterEventClass($entity);
        $this->container->get('event_dispatcher')->dispatch(constant('\\MU\\VideoModule\\VideoEvents::' . strtoupper($entity->get_objectType()) . '_POST_UPDATE'), $event);
    }
    ...
    public function postLoad(LifecycleEventArgs $args)
    {
        ...
        $filterEventClass = '\\MU\\VideoModule\\Event\\Filter' . ucfirst($entity->get_objectType()) . 'Event';
        $event = new $filterEventClass($entity);
        $this->container->get('event_dispatcher')->dispatch(constant('\\MU\\VideoModule\\VideoEvents::' . strtoupper($entity->get_objectType()) . '_POST_LOAD'), $event);
    }
    ...
    protected function isEntityManagedByThisBundle($entity)
    {
        ...
        return ($entityClassParts[0] == 'MU' && $entityClassParts[1] == 'VideoModule');
    }
    ...
    protected function getUploadFields($objectType)
    {
        ...
        switch ($objectType) {
            case 'movie':
                $uploadFields = ['uploadOfMovie', 'poster'];
                break;
        }
