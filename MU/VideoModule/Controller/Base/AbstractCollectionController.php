...
abstract class AbstractCollectionController extends AbstractController
{
    ...
    protected function indexInternal(Request $request, $isAdmin = false)
    {
        ...
        return $this->redirectToRoute('muvideomodule_collection_' . $templateParameters['routeArea'] . 'view');
    }
    /**
    ...
    protected function viewInternal(Request $request, $sort, $sortdir, $pos, $num, $isAdmin = false)
    {
        ...
        $sortableColumns = new SortableColumns($this->get('router'), 'muvideomodule_collection_' . ($isAdmin ? 'admin' : '') . 'view', 'sort', 'sortdir');
        ...
        foreach ($templateParameters['items'] as $collection) {
            if (!$this->hasPermission('MUVideoModule:' . ucfirst($objectType) . ':', $collection->getKey() . '::', $permLevel)) {
                continue;
            }
    ...
    protected function displayInternal(Request $request, CollectionEntity $collection, $isAdmin = false)
    {
        ...
        $featureActivationHelper = $this->get('mu_video_module.feature_activation_helper');
        if ($featureActivationHelper->isEnabled(FeatureActivationHelper::CATEGORIES, $objectType)) {
            if (!$this->get('mu_video_module.category_helper')->hasPermission($collection)) {
                throw new AccessDeniedException();
            }
        }
    ...
    protected function deleteInternal(Request $request, CollectionEntity $collection, $isAdmin = false)
    {
        ...
        $redirectRoute = 'muvideomodule_collection_' . ($isAdmin ? 'admin' : '') . 'view';
    ...
    protected function handleSelectedEntriesActionInternal(Request $request, $isAdmin = false)
    {
        ...
        foreach ($items as $itemId) {
            ...
            if ($action == 'delete') {
                $this->addFlash('status', $this->__('Done! Item deleted.'));
                $logger->notice('{app}: User {user} deleted the {entity} with id {id}.', ['app' => 'MUVideoModule', 'user' => $userName, 'entity' => 'collection', 'id' => $itemId]);
            } else {
        ...
        return $this->redirectToRoute('muvideomodule_collection_' . ($isAdmin ? 'admin' : '') . 'index');
    }
}
