 ...
 */
function MUVideoModule_needleapi_collection_base($args)
{
    ...
    if ($needleId == 'COLLECTIONS') {
        if (!$permissionApi->hasPermission('MUVideoModule:Collection:', '::', ACCESS_READ)) {
            $cache[$nid] = '';
    ...
    $cache[$nid] = '<a href="' . $router->generate('muvideomodule_collections_view') . '" title="' . $translator->__('View collections') . '">' . $translator->__('Collections') . '</a>';
    ...
    if (!$permissionApi->hasPermission('MUVideoModule:Collection:', $entityId . '::', ACCESS_READ)) {
        $cache[$nid] = '';

        return $cache[$nid];
    }

    $repository = $container->get('mu_video_module.entity_factory')->getRepository('collection');
    $entity = $repository->selectById($entityId);
    if (null === $entity) {
        $cache[$nid] = '<em>' . $translator->__f('Collection with id %id% could not be found', ['%id%' => $entityId]) . '</em>';

        return $cache[$nid];
    }

    $title = $entity->getTitleFromDisplayPattern();
    $cache[$nid] = '<a href="' . $router->generate('muvideomodule_collections_display', ['id' => $entityId]) . '" title="' . str_replace('"', '', $title) . '">' . $title . '</a>';
