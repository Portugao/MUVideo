...
namespace MU\VideoModule\Entity\Repository\Base;
 ...
 *
 * This is the base repository class for playlist entities.
 */
abstract class AbstractPlaylistRepository extends EntityRepository
{
    ...
    protected function getViewQuickNavParameters($context = '', $args = [])
    {
        ...
        $parameters['catIdList'] = $categoryHelper->retrieveCategoriesFromRequest('playlist', 'GET');
    ...
    public function truncateTable(LoggerInterface $logger)
    {
        ...
        $qb->delete('MU\VideoModule\Entity\PlaylistEntity', 'tbl');
        $query = $qb->getQuery();
    
        $query->execute();
    
        $logArgs = ['app' => 'MUVideoModule', 'entity' => 'playlist'];
    ...
    public function updateCreator($userId, $newUserId, TranslatorInterface $translator, LoggerInterface $logger, CurrentUserApi $currentUserApi)
    {
        ...
        $qb = $this->getEntityManager()->createQueryBuilder();
        $qb->update('MU\VideoModule\Entity\PlaylistEntity', 'tbl')
        ...
        $logArgs = ['app' => 'MUVideoModule', 'user' => $currentUserApi->get('uname'), 'entities' => 'playlists', 'userid' => $userId];
     ...
     *
     * @return array|playlistEntity retrieved data array or playlistEntity instance
     *
     * @throws InvalidArgumentException Thrown if invalid parameters are received
     */
     ...
     * @param boolean $slimMode If activated only some basic fields are selected without using any joins (optional) (default=false)
     *
     * @return ArrayCollection collection containing retrieved playlistEntity instances
     *
     * @throws InvalidArgumentException Thrown if invalid parameters are received
     */
    ...
    public function addCommonViewFilters(QueryBuilder $qb)
    {
        ...
        foreach ($parameters as $k => $v) {
            if ($k == 'catId') {
                 ...
                 */
                $categoryHelper = \ServiceUtil::get('mu_video_module.category_helper');
                $qb = $categoryHelper->buildFilterClauses($qb, 'playlist', $v);
    ...
    {
        $selection = 'COUNT(tbl.id) AS numPlaylists';
        if (true === $useJoins) {
        ...
        $qb = $this->getEntityManager()->createQueryBuilder();
        $qb->select($selection)
           ->from('MU\VideoModule\Entity\PlaylistEntity', 'tbl');
     ...
     * @param integer $excludeId  Id of playlists to exclude (optional)
     *
     * @return boolean result of this check, true if the given playlist does not already exist
     */
    ...
    protected function genericBaseQueryAddWhere(QueryBuilder $qb, $where = '')
    {
        if (!empty($where) || null !== $this->getRequest()) {
            ...
            $categoryHelper = \ServiceUtil::get('mu_video_module.category_helper');
            $categoryProperties = $categoryHelper->getAllProperties('playlist');
    ...
    public function getQueryFromBuilder(QueryBuilder $qb)
    {
        ...
        if ($featureActivationHelper->isEnabled(FeatureActivationHelper::TRANSLATIONS, 'playlist')) {
