...
use MU\VideoModule\Helper\ModelHelper;
...
abstract class AbstractControllerHelper
{
    ...
    /**
     * Returns an array of all allowed object types in MUVideoModule.
     *
    ...
    /**
     * Returns the default object type in MUVideoModule.
     *
     * @param string $context Usage context (allowed values: controllerAction, api, helper, actionHandler, block, contentType, util)
     * @param array  $args    Additional arguments
     *
    ...
    public function processViewActionParameters($objectType, SortableColumns $sortableColumns, array $templateParameters = [], $hasHookSubscriber = false)
    {
        ...
        $templateParameters['own'] = $request->query->getInt('own', $this->variableApi->get('MUVideoModule', 'showOnlyOwnEntries', 0));
        ...
        if ($templateParameters['all'] != 1) {
            ...
            if (in_array($resultsPerPage, [0, 10])) {
                $resultsPerPage = $this->variableApi->get('MUVideoModule', $objectType . 'EntriesPerPage', 10);
            }
        }
        ...
        $quickNavForm = $this->formFactory->create('MU\VideoModule\Form\Type\QuickNavigation\\' . ucfirst($objectType) . 'QuickNavType', $templateParameters);
        ...
        if (true === $hasHookSubscriber) {
            ...
            $templateParameters['currentUrlObject'] = new RouteUrl('muvideomodule_' . strtolower($objectType) . '_view', $urlParameters);
        }
    ...
    public function processDisplayActionParameters($objectType, array $templateParameters = [], $hasHookSubscriber = false)
    {
        ...
        if (true === $hasHookSubscriber) {
            ...
            $templateParameters['currentUrlObject'] = new RouteUrl('muvideomodule_' . strtolower($objectType) . '_display', $urlParameters);
        }
    ...
    public function addTemplateParameters($objectType = '', array $parameters = [], $context = '', array $args = [])
    {
        ...
        if ($context == 'controllerAction') {
            ...
            if ($objectType == 'movie') {
                $thumbRuntimeOptions = [];
                $thumbRuntimeOptions[$objectType . 'UploadOfMovie'] = $this->imageHelper->getRuntimeOptions($objectType, 'uploadOfMovie', $context, $args);
                $thumbRuntimeOptions[$objectType . 'Poster'] = $this->imageHelper->getRuntimeOptions($objectType, 'poster', $context, $args);
            ...
            if (in_array($args['action'], ['display', 'edit', 'view'])) {
                ...
                $parameters['relationThumbRuntimeOptions'] = $this->imageHelper->getCustomRuntimeOptions('', '', 'MUVideoModule_relateditem', $context, $args);
            }
        }
