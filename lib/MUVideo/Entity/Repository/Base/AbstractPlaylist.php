 ...
 */
abstract class MUVideo_Entity_Repository_Base_AbstractPlaylist extends EntityRepository
{
    /**
    ...
    public function getAdditionalTemplateParameters($context = '', $args = array())
    {
        ...
        if ($context == 'controllerAction') {
            ...
            if (in_array($args['action'], array('main', 'view'))) {
                ...
                $templateParameters['workflowStateItems'] = $listHelper->getEntries('playlist', 'workflowState');
            }
    ...
    public function truncateTable()
    {
        ...
        $qb->delete('MUVideo_Entity_Playlist', 'tbl');
        $query = $qb->getQuery();
    ...
    public function updateCreator($userId, $newUserId)
    {
        ...
        $qb = $this->getEntityManager()->createQueryBuilder();
        $qb->update('MUVideo_Entity_Playlist', 'tbl')
    ...
    public function getSelectWherePaginatedQuery(QueryBuilder $qb, $currentPage = 1, $resultsPerPage = 25)
    {
        ...
        $paginatorClass = 'MUVideo_Paginator_' . (!$isLegacy ? 'Paginator' : 'LegacyPaginator');
    ...
    public function selectWherePaginated($where = '', $orderBy = '', $currentPage = 1, $resultsPerPage = 25, $useJoins = true, $slimMode = false)
    {
        ...
        if (!$hasFilters) {
            if ($page > 1 || isset($_GET['pos'])) {
                ...
                SessionUtil::setVar('MUVideoPlaylistsCurrentPage', $page);
            } else {
                // restore current page from session
                $page = SessionUtil::getVar('MUVideoPlaylistsCurrentPage', 1);
    ...
    protected function applyDefaultFilters(QueryBuilder $qb, $parameters = array())
    {
        ...
        if ($currentLegacyControllerType == 'admin' && $currentModule == 'MUVideo') {
            return $qb;
        }
    
        if (!in_array('workflowState', array_keys($parameters)) || empty($parameters['workflowState'])) {
            // per default we show approved playlists only
    ...
    protected function addSearchFilter(QueryBuilder $qb, $fragment = '')
    {
        ...
        if (!$fragmentIsNumeric) {
            ...
            $where .= ((!empty($where)) ? ' OR ' : '');
            $where .= 'tbl.urlOfYoutubePlaylist = \'' . $fragment . '\'';
        } else {
            $where .= ((!empty($where)) ? ' OR ' : '');
            $where .= 'tbl.title LIKE \'%' . $fragment . '%\'';
            $where .= ((!empty($where)) ? ' OR ' : '');
    ...
    public function retrieveCollectionResult(Query $query, $orderBy = '', $isPaginated = false)
    {
        ...
        if ($isLegacy) {
            ...
            if (!$isPaginated) {
                ...
                $paginatorClass = 'MUVideo_Paginator_Paginator';
                $hasRelationships = true;
                $paginator = new $paginatorClass($query, $hasRelationships);
    ...
    protected function getCountQuery($where = '', $useJoins = true)
    {
        ...
        $qb = $this->getEntityManager()->createQueryBuilder();
        $qb->select($selection)
           ->from('MUVideo_Entity_Playlist', 'tbl');
     ...
     * @param int    $excludeId  Id of playlists to exclude (optional)
     *
