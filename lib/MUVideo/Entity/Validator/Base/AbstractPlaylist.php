 ...
 *
 * This is the base validation class for playlist entities.
 */
abstract class MUVideo_Entity_Validator_Base_AbstractPlaylist extends MUVideo_Validator
{
    /**
    ...
    public function validateAll()
    {
        ...
        if (!$this->isStringNotLongerThan('urlOfYoutubePlaylist', 255)) {
            $errorInfo['message'] = __f('Error! Length of field value must not be higher than %2$s (%1$s).', array('url of youtube playlist', 255), $dom);
            return $errorInfo;
        }
        if (!$this->isStringNotEmpty('urlOfYoutubePlaylist')) {
            $errorInfo['message'] = __f('Error! Field value must not be empty (%s).', array('url of youtube playlist'), $dom);
            return $errorInfo;
        }
        if (!$this->isValidUrl('urlOfYoutubePlaylist')) {
            $errorInfo['message'] = __f('Error! Field value must be a valid url (%s).', array('url of youtube playlist'), $dom);
            return $errorInfo;
        }
     ...
     *
     * This method determines if there already exist playlists with the same playlist.
     *
